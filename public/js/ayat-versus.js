 ...
 *  - BUG #6: Added 10s timeout fallback in checkRoundComplete to prevent deadlocks
...
const VersusAyat = (() => {
  let state = {
    isActive: false,
    _originalQuestions: [], // Keep original pool for rematch re-shuffle
    questions: [],
    currentIndex: 0,
  ...
  function init(allQuestions) {
    ...
    Swal.fire({
      ...
      state._originalQuestions = [...allQuestions]; // Save pool for rematch
      _startRound();
    });
  }

  /**
   * FIX #3 & #4: Internal start function — resets and starts game WITHOUT Swal prompt.
   * Called by both first-start (after Swal) and rematch.
   */
  function _startRound() {
    state.isActive = true;
    // FIX: Always re-shuffle for new round
    state.questions = shuffleArray([...state._originalQuestions]).slice(0, 10);
    state.currentIndex = 0;

    state.p1 = { score: 0, ready: false, name: state.p1.name };
  ...
  // FIX #3: rematch no longer calls init() — directly starts a new round
  function rematch() {
  ...
  function loadQuestion() {
    ...
    // FIX #6: Start deadlock prevention timer — if 30s pass with no resolution, auto-advance
    state._deadlockTimeout = setTimeout(() => {
      if (state.isActive && !state.isTransitioning) {
        console.warn("[VersusAyat] Deadlock timeout — advancing to next question");
  ...
  function renderPlayerUI(playerUI, q, playerId) {
    ...
    options.forEach((opt) => {
      ...
      // FIX #7: Use { once: true } on touchstart to prevent double-fire with click
  ...
  function handleAnswer(playerId, selected, correct, btnElement) {
    ...
    if (isCorrect) {
      ...
      // Disable other player's buttons immediately
      ...
      if (otherUI) {
        ...
        // Show "too late" indicator to other player
        if (otherUI.question) {
    ...
    if (isCorrect) {
      ...
      // FIX #6: Mark this player as done with wrong answer
  ...
  function checkRoundComplete() {
    // Both players answered wrong → advance to next question
